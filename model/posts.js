...
Post.getPosts = function(endPost, userId, count, callback){
    endPost = endPost || "ffce5eef0000000000000000";
    count = count || 60;
    const now = new Date();
    ...
    const projection1 = {
        ...
        timediff : {$add : [{$subtract : [now, '$postDate']}, nowMilli]},
        postLikeUsers : 1,
        replyCount : 1,
        postLikeCount : 1,
        likedByMyFriend : {
            $setIntersection : ['$postLikeUsers', '$userInfo.friendList']
        },
        writtenByMyFriend : {
            $cond : [
                {$setIsSubset : [['$userId'], '$userInfo.friendList']},
                true,
                false
            ]
        },
        repliedByMyFriend : {
            $setIntersection : [['$reply.userId'], '$userInfo.friendList']
        },
        userInfo : 1
    }
    const format = { $cond : [
    ...
    const projection2 = {
        ...
        postDate : {
            $let : {
                vars : {
                    year : {$year : '$timediff'},
                    month : {$month : '$timediff'},
                    day : {$dayOfMonth : '$timediff'},
                    hours : {$hour : '$timediff'},
        ...
        myLike : {
            $cond : [
                {$setIsSubset : [[mongoose.Types.ObjectId(userId)], '$postLikeUsers']},
                true,
                false
            ]
        }
    ...
    post.aggregate()
        ...
        .project(projection0)
        .lookup({from : 'users', localField : 'currentUserId', foreignField : '_id', as : 'userInfo'})
        .unwind('userInfo')
        .project(projection1)
        .match({ $or : [{writtenByMyFriend : true}, {repliedByMyFriend : {$ne : []}}, {likedByMyFriend : {$ne : []}}]})
        .project(projection2)
        .sort({_id : -1})
        .limit(count)
        .then((docs)=>{
            if (docs[0]){
                callback(null, docs, docs[0].hasFriend);
            } else {
